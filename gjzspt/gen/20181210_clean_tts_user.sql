...
WHERE
    T1.ENTITY_IDCODE IN ( SELECT T2.ENTITY_IDCODE FROM TTS_SCLTXXCJ_REGISTER_V2 T2 WHERE T2.ACCOUNT IN ( 'yunhongjituan' ) );
...
WHERE
    T1.ENTITY_ID_CODE IN ( SELECT T2.ENTITY_IDCODE FROM TTS_SCLTXXCJ_REGISTER_V2 T2 WHERE T2.ACCOUNT IN ( 'yunhongjituan' ) );
...
WHERE
    T1.FROM_ID IN ( SELECT T2.ENTITY_IDCODE FROM TTS_SCLTXXCJ_REGISTER_V2 T2 WHERE T2.ACCOUNT IN ( 'yunhongjituan' ) );
...
WHERE
    ACCOUNT IN ( SELECT ACCOUNT FROM TTS_SCLTXXCJ_REGISTER_V2 WHERE ACCOUNT IN ( 'yunhongjituan' ) );
...
WHERE
    ACCOUNT IN ( 'yunhongjituan' );	
--　删除用户帐号
DELETE
FROM
    SYS_USER S
WHERE
S.ACCOUNT in ('yunhongjituan');        
